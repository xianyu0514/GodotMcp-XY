...
Tools that drive the Godot editor itself: running/stopping the project, reading editor state, capturing viewport screenshots, inspecting signals, selecting nodes and files, the Inspector-style property view, export preset/template management, and editor buffer synchronisation (unsaved changes, save/reload/close scripts).

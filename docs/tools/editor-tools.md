...
| `manage_export_templates` | advanced | Manage locally installed Godot export templates: report status (templates dir installed versions and the official download URL/.tpz filename); `download` starts a trusted background download of the current version's templates from an allowlisted official mirror (github/tuxfamily/godotengine) — optionally through a loopback HTTP proxy (system proxy detected automatically; Godot's HTTPRequest has no proxy support, so the tool ships its own CONNECT-tunnel TLS client) — cross-checks the official asset byte size from the GitHub release API and auto-installs on completion (`pending` → poll with `download_status`, cancel with `download_cancel`); install a local .tpz/.zip archive or remove an installed version directory; `net_diag` dials a hostname, an IP literal and a control IP in parallel from the editor process, reporting per-peer status timelines plus `_process` tick/frame counts to distinguish DNS hangs, outbound blocks and stalled node processing. Downloads temporarily enable the editor's *Update Continuously* setting so they keep pumping while the editor window is unfocused, restoring the previous value on completion. This is the tool-side trusted network path: ScriptSandbox restrictions on `execute_script` never apply. Works on Godot 4.6+. |

...
1. For a complete game outcome, call `run_game_workflow` with `command`. If no plan exists it is created; repeat the same command after yields/restarts to resume without duplicating completed work. Use `plan_game_workflow` when explicit inspect/replan/cancel control is needed.

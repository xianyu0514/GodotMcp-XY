...
| `search_tools` | meta | `mode=tools` ranks exact candidates; `mode=workflow` builds a compact, cost-aware inspect/execute/verify route. Both paths are deterministic and local. Workflow results contain names plus three aggregate token-cost metrics, default to 8 tools, hard-cap at 10 and cover all 227 non-meta atomic tools through the immutable bilingual index. Unfiltered routes reuse a 64-entry LRU; filters remain isolated. |

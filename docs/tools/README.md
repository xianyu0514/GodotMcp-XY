...
Godot MCP Native registers **234 MCP tools**: 28 core tools, 200 advanced tools and 6 always-on meta tools. The manifest in `addons/godot_mcp/native_mcp/tools_manifest.gd` is the source of truth for tier and group membership.
...
| Project | 71 | 3 | 68 | [project-tools.md](project-tools.md) |
...
6. Use `search_tools` only to preview/compare, `get_tool_details` only when the client cannot refresh, and `list_tool_catalog(summary_only=true)` only to browse groups and the full coverage summary.
...
| Project | `project_tools_native.gd`（主）+ `project_resources_tools.gd` / `project_assets_tools.gd` / `project_tileset_tools.gd` / `project_verification_tools.gd` / `project_workflow_tools.gd` / `dependency_impact_tools.gd` |
...
Existing-script edits additionally support unique text replacement and optional content-hash preconditions, and reject known unsaved target buffers. Invalid line edits preserve the file. Counts remain 234 tools (28 core, 200 advanced, 6 meta).

...
Godot MCP Native registers **242 MCP tools**: 28 core tools, 208 advanced tools and 6 always-on meta tools. The manifest in `addons/godot_mcp/native_mcp/tools_manifest.gd` is the source of truth for tier and group membership.
...
| Debug & Runtime | 77 | 3 | 74 | [debug-tools.md](debug-tools.md) |
...
Existing-script edits additionally support unique text replacement and optional content-hash preconditions, and reject known unsaved target buffers. Invalid line edits preserve the file. Counts remain 242 tools (28 core, 208 advanced, 6 meta).

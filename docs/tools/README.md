...
Godot MCP Native registers **251 MCP tools**: 28 core tools, 217 advanced tools and 6 always-on meta tools. The manifest in `addons/godot_mcp/native_mcp/tools_manifest.gd` is the source of truth for tier and group membership.
...
| Scene | 18 | 4 | 14 | [scene-tools.md](scene-tools.md) |
...
Existing-script edits additionally support unique text replacement and optional content-hash preconditions, and reject known unsaved target buffers. Invalid line edits preserve the file. Counts remain 251 tools (28 core, 217 advanced, 6 meta).

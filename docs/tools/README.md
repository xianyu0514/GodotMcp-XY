...
Godot MCP Native registers **244 MCP tools**: 28 core tools, 210 advanced tools and 6 always-on meta tools. The manifest in `addons/godot_mcp/native_mcp/tools_manifest.gd` is the source of truth for tier and group membership.
...
| Scene | 15 | 4 | 11 | [scene-tools.md](scene-tools.md) |
...
Existing-script edits additionally support unique text replacement and optional content-hash preconditions, and reject known unsaved target buffers. Invalid line edits preserve the file. Counts remain 244 tools (28 core, 210 advanced, 6 meta).

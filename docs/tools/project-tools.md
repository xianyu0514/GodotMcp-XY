...
| `assert_visual_baseline` | Advanced | Visual regression gate: compare a candidate screenshot against a stored baseline (golden) image and pass/fail against tolerances. Bootstraps the baseline from the candidate when missing (or `update_baseline=true`); otherwise passes only when `diff_pixel_count <= max_diff_pixels` and (when set) `diff_ratio <= max_diff_ratio` and `rmse <= rmse_threshold`. Can write a diff heatmap to `diff_output_path`. |

...
| `run_verification_queue` | advanced | Create, advance, inspect, record into, or abandon a persistent sliced verification queue: each advance consumes at most `budget` pending items (the rest retained, never dropped), evidence is fingerprinted against `watch_paths` (file drift pushes stale verdicts back to pending), `completed` only when every item passed. `script_check` items run the built-in GDScript compile check; `behavior_check` items are executed NATIVELY by the queue (probe -> run_project -> play_and_verify steps/assertions -> stop) and record `native_run` evidence (scene, per-assertion actual/expected, runtime errors, screenshots, session id); `external` items run out-of-band, verdicts come back via `command=record` and are marked `external_claim`; `strict=true` queues reject externally recorded verdicts — native evidence only. Restart-safe. |
| `get_game_project_brief` | advanced | One call rebuilds the session context: project identity (name, main scene, input actions), content volume, task-plan state (per-status counts + active tasks), verification queues with UNverified requirements named one by one, recent change-journal entries, and derived next sentences (plan it / unblock it / close the gap / tune-add-ship / make_first_game). Read-only; replaces the ~10-call state re-discovery at the top of every session. |

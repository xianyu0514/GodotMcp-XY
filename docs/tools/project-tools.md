...
| --- | --- | --- |
| `apply_project_change_set` | core | Atomically preflight, revision-guard, apply and verify coordinated `file_write`, `project_setting`, `node_properties` and `scene_nodes` changes. `dry_run` returns a revision; conflicts reject before writing, and later failures restore file/settings changes plus UndoRedo-backed scene edits. |
| `get_project_info` | core | Get general information about the Godot project, including name, version, and description. |
| `get_project_context` | core | Return compact context with whole-content and per-section SHA-256 revisions. `known_revision` suppresses an unchanged response; `known_section_revisions` returns only changed project/editor/autoload/input/global-class/task-plan/tool sections. |
...
| `manage_task_plan` | advanced | Persistent task graph + DoD gates with recoverable execution. `claim` leases ready work, `checkpoint` stores structured progress, `fail` records evidence and retries or blocks, and `resume` finds active/expired tasks. |

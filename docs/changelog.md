...
- Fixed a `set_dod` rollback bug on the new-criterion path: when creating a criterion by `criterion` text, all error conditions (invalid gate, `observed` without a gate, non-dict `observed`) are now validated before the criterion is appended, so a failed call no longer leaves a half-created criterion in the task's DoD.

...
- Fixed `set_dod` to store criterion text trimmed, matching how criteria are matched and created. Previously a `criterion` passed with surrounding whitespace (e.g. `"  fps ok  "`) was matched/created using the trimmed text but persisted with the padding, which could cause surprising inconsistencies; now the stored text is trimmed too.

...
> 用于指导 **Godot-MCP-Native**（纯 GDScript、Godot 4.7 编辑器内原生 MCP 服务器，232 工具）的优化。
...
**第一题结论（诚实口径）**：在该产物类上两条工具链**表达力打平**（判据
数字完全一致——同时证明判据中立）；我方优势在调用经济性（-38% 调用、
零死胡同）与验证深度（帧定时/天梯/A 层对方无对等物）；对方优势在 3D
广度（CSG/GridMap）与批量撤销。**不是全面碾压，是维度分胜**。第二题
（平台跳跃，压我方矩阵强项）与第三题（juice）仍待执行。

...
## 2026-09-03 — 迭代目标失忆：第二个目标建平行文件（fixed）

- **客户端**：连续两个 gameplay 目标（先"做移动控制器"，后"给玩家加二段跳"）
- **现象**：工件注册表只活在单个 plan 里，第二个目标对第一个的产物失忆，
  会再建 `gameplay-feature-wf_007.gd` 之类的平行新文件，`read_script` 回退
  取目录第一个脚本；"改现有玩法"被当成"再做一个"。
- **修复**：项目状态账本（`res://.mcp/project_state.json`）持久化工件与
  语义套件；第二个 gameplay 目标默认迭代模式（打开/读取/修改 + 语义回归），
  上一目标的行为测试自动重跑。
- **状态**：fixed（本次提交）。

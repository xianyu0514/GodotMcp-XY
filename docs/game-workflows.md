...
`plan_game_workflow` and `run_game_workflow` are the compact server-side loop for goals that cannot be completed reliably by selecting one small tool set once. They do not replace the 228 atomic capabilities. They compose and execute those capabilities through a persistent, evidence-gated plan.
...
- **Goal semantic evidence.** Blueprint-verb goals (movement/jump/collectible/win) additionally generate a `McpGameTestSuite` under `res://tests/game/` and execute it as an objective gate through `run_game_tests` (headless subprocess with watchdog). The gate asserts the objective's behavior — input displaces the character, jump launches upward, the coin is reached and collected, the win label appears — so `completed` proves the goal happened, not merely that nothing errored. The suite registers its own InputMap fallbacks and stays in the project as a regression asset.

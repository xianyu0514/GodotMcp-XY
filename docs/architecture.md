...
Eleven pre-normalized game-development seeds plus an adaptive fallback greedily cover intent terms and emit only a bounded list of names grouped into inspect/execute/verify stages. Selection first maximizes newly covered intent terms. Within equal coverage, it compares semantic score per incremental schema token with integer cross multiplication, then lower cost and lexical name for byte-stable ties. Exact names and complete official English/Chinese descriptions keep strict priority regardless of cost, which makes all 217 atomic capabilities directly routable while the default workflow payload remains capped at 8 names (hard maximum 10). Every route exposes three compact diagnostics: supplementary schema tokens added, supplementary tokens for a full load and the normalized savings ratio. The full-catalog regression currently measures 506 versus 26,771 estimated tokens (98.11% avoided) for the representative debug/verify/performance goal.

...
Godot-MCP 实现了 **201 个工具**，分为 6 大类和两种级别：

**核心工具（30 个）**：默认启用，始终可见。
**补充工具（171 个）**：需在工具管理面板中手动启用。

| 类别 | 核心 | 补充 | 总计 | 说明 |
|------|------|------|------|------|
| Node Tools | 9 | 17 | 26 | 节点管理（创建、删除、修改属性、复制、移动、重命名、信号、组、批量读取/连接、Control 偏移变换、2D 单向碰撞、内联子资源设置/读取） |
| Script Tools | 7 | 10 | 17 | 脚本管理（读取、批量读取、修改、创建、分析、符号索引、搜索、GDScript 验证、着色器校验） |
| Scene Tools | 4 | 8 | 12 | 场景管理（创建、保存、打开、结构查看、实例化预制场景、节点分支另存为场景、TileMapLayer 单元格设置/读取） |
| Editor Tools | 4 | 19 | 23 | 编辑器操作（运行、停止、截图、导出、选择、缓冲区同步、导入状态） |
| Debug Tools | 3 | 67 | 70 | 调试和运行时（日志、断点、栈帧、Profiler、运行时探针、动画/音频/着色器/瓦片地图控制） |
| Project Tools | 3 | 50 | 53 | 项目配置（信息、设置、资源、自定义/批量资源创建与属性读写、测试、输入映射、自动加载、全局类、诊断、反向资源依赖、迁移检查、弃用 API 扫描、GDExtension 检测、渐变纹理创建、PCK 打包、渲染输出配置、可绘制纹理创建与绘制、UI 主题创建与设置、项目设置写入、自动加载增删、动画资源创建与关键帧插入、TileSet 创建与图层配置、逐图块碰撞多边形/地形设置） |

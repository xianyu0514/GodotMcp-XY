...
- **目标语义门禁**：命中动词的目标会额外生成一个 `McpGameTestSuite`（`res://tests/game/gameplay_semantic.gd`）并作为 objective gate 执行——断言"输入会位移、跳跃会升起、金币被走到并吃到、胜利标签随后出现"。`completed` 因此证明的是目标行为真的发生，而不只是"没有报错"。这些行为测试留在用户项目里，成为可持续回归资产。动词覆盖：移动/跳跃/收集/胜利/**敌人（追逐）**/**计分**——计分蕴含收集循环（ScoreLabel 随拾取实时更新）；**射击**类目标只做分类与移动载体，弹道逻辑由客户端凭 `content_brief` 创作。

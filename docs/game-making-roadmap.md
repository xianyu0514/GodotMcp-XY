...
## 3. M2 — 注意力里程碑：注意力花在游戏上，不花在管道上（2026-09-23 重定义）

**判断依据（本会话实测）**：注意力的最大去向是两类"管道税"——① 每个会话开头的
状态重建（本会话花了十几个调用搞清仓库状态；游戏项目每个会话都要交同等的税：
做完了什么/验证到哪/下一步是什么）；② 首次接触的脚手架即兴（first_playable
链路有效但每次重拼）。验证类注意力**不算税**——那正是在了解游戏。
不可度量则不可优化：注意力指标是本里程碑的基石。

四个工作包（按依赖序）：

**WP1 `make_first_game` 入口配方**（最快见效，产品化已验证链路）：
`make_first_game(goal, genre=platformer|top_down|shooter)` — 脚手架（输入映射→玩家→
一张图→一个敌人→胜负条件）→ 严格契约（移动/撞墙/击败/胜负全 FRESH 断言）→
截图汇报 → 收尾指向支柱配方与续会话工具。genre 差异是数据不是分支（stats 先例）。

**WP2 `get_game_project_brief` 工具**（消除最大经常性税，一次调用重建全部上下文）：
单调用返回会话简报——项目身份/主场景/输入动作数、场景与脚本清单、任务图状态
（进行中+DoD 门）、验证队列状态（逐项点名未验证需求）、最近变更日志、
**建议的下三句话**。答案同行（E-2）：证据指针内嵌。走工具八步出厂 +
宪章合规（S-1 自披露：无任务图时建议 plan_game_feature 而非报错）。

**WP3 注意力指标进 plugin-user release 流程**（度量基石）：
- TTFP：装插件 → 契约 COMPLETE 的代理轮次与墙钟时间，写入测试输出；
- 注意力占比：游戏内容轮次 / 总轮次（按调用分类：内容创作 vs 发现/编排/重读），
  先记录基线、后设回归门禁。

**WP4 快赢（宪章待办，同样是注意力节省）**：K-1 审计 21 个配方剔除通用常识填充；
E-3 存量文本真理下沉清单（每条标注候选工具分支）。

**DoD**：全新装插件 → `make_first_game` → 契约 COMPLETE，TTFP 基线落档；
"会话二"场景：`get_game_project_brief` 一次调用 + 一句调参指令端到端完成
（对比现状 ~10 调用的状态重建）；注意力占比首份基线报告产出；
全量 GUT 0 失败；宪章门禁全绿。
...
| TTFP（装插件→契约 COMPLETE） | 有链路无入口配方 | — | 有配方 + 计时 + 注意力占比基线 | — |
| 假完成率（故障注入下） | 0 | 0 | 0 | 0 |
...
## 8.7 "完美游戏"战略（2026-09-23 增补）

用户目标升级：任何游戏 → **完美游戏**。落地为 [`docs/perfect-game-ladder.md`](perfect-game-ladder.md)
四级天梯（可玩/扎实/打磨/完美）+ `make_game_perfect` 配方（第 26 个）：
M 项全复用既有测量设施（输入延迟=每帧轨迹的首变帧、公平性=前摇帧数、反馈覆盖/密度=审计字段），
A 项（视觉一致/平衡/前 30 秒上手）必须带截图证据被评审；R4 声称 = 全绿或显式豁免，
否则输出精确缺口清单。零新工具（宪章 K）。

## 8.8 完美游戏执行规划（2026-09-23 定稿）

> 前置：天梯标准（docs/perfect-game-ladder.md）与 make_game_perfect 配方已交付。
> 本规划把"完美"从**标准**推进到**一键可测、真机有证、持续有基线**。

**WP1 天梯真机验收（第一优先）** — 标准没被真机测过就不算标准：
- 扩展 test_l3_any_game_flow.py（高尔夫）：延迟项 = frame0 按下 + 每帧采样 ball_x，
  **测试侧从轨迹计算首变帧**并断言 ≤3 帧、≥1 帧；反馈覆盖 = launch 事件有 Burst 粒子响应（1/1）；
  公平性/持久化按豁免纪律显式记录理由（该游戏无伤害源/无存档——豁免本身是验收点）。
- ✅ 已完成（基准首跑合并执行）：延迟实测 **2 帧（~33ms@60Hz）** 基线落档；新发现：
  user:// 存档跨运行残留是集成 flake 源（测试启动前必须清 appdata，已写入基准驱动）。
  详见 test_benchmark_voxel_flow.py。

**WP2 `game_quality_ladder` 汇总工具（"轻松"的核心）** — 一次调用测齐全部 M 级：
- 输入 hints（generic 工具无法自寻目标）：movement{action,node,property}、
  threats[{node,windup_property}]、events[...]；编排多次 FRESH 启动跑齐
  R1-R4 的全部 M 项，返回**天梯状态**（每项带证据；A 项标 awaiting_review + 截图路径）。
...
**暂缓声明**：M8 分发 UX（一键客户端配置/签名发布/零遥测声明）仍在队列，不因本规划消失；
本地 3 个提交待代理恢复推送。

...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（241/28/207/6）
...
func test_all_231_tools_registered():
	...
	assert_eq(all_tools.size(), 241, "Should have exactly 241 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 207, "Should have 207 supplementary tools")
...
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 241, "manifest 应包含 241 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 28, "manifest 应有 28 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 207, "manifest 应有 207 个 supplementary 工具")

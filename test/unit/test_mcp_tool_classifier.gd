...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（248/28/214/6）
...
func test_all_231_tools_registered():
	...
	assert_eq(all_tools.size(), 248, "Should have exactly 248 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 214, "Should have 214 supplementary tools")
...
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 248, "manifest 应包含 248 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 28, "manifest 应有 28 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 214, "manifest 应有 214 个 supplementary 工具")

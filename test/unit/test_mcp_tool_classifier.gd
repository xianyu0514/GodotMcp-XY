...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（249/28/215/6）
...
func test_all_231_tools_registered():
	...
	assert_eq(all_tools.size(), 249, "Should have exactly 249 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 215, "Should have 215 supplementary tools")
...
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 249, "manifest 应包含 249 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 28, "manifest 应有 28 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 215, "manifest 应有 215 个 supplementary 工具")

...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（238/28/204/6）
...
func test_all_231_tools_registered():
	...
	assert_eq(all_tools.size(), 238, "Should have exactly 238 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 204, "Should have 204 supplementary tools")
...
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 238, "manifest 应包含 238 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 28, "manifest 应有 28 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 204, "manifest 应有 204 个 supplementary 工具")

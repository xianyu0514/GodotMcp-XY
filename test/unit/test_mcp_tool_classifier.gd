...
func test_all_191_tools_registered():
	var all_tools: Array = _classifier.get_all_tools()
	assert_eq(all_tools.size(), 191, "Should have exactly 191 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 161, "Should have 161 supplementary tools")

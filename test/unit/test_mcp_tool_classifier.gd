...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（234/28/200/6）
...
func test_all_231_tools_registered():
	...
	assert_eq(all_tools.size(), 234, "Should have exactly 234 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 200, "Should have 200 supplementary tools")
...
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 234, "manifest 应包含 234 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 28, "manifest 应有 28 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 200, "manifest 应有 200 个 supplementary 工具")

...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（233/28/199/6）
...
func test_all_231_tools_registered():
	...
	assert_eq(all_tools.size(), 233, "Should have exactly 233 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 199, "Should have 199 supplementary tools")
...
## manifest 计数基线：233 总 / 28 core / 199 supplementary / 6 meta。
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 233, "manifest 应包含 233 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 28, "manifest 应有 28 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 199, "manifest 应有 199 个 supplementary 工具")

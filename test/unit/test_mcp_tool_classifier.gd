...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（225/30/191/4）
...
func test_all_225_tools_registered():
	var all_tools: Array = _classifier.get_all_tools()
	assert_eq(all_tools.size(), 225, "Should have exactly 225 tools registered")
...
func test_get_project_context_is_core_project_tool():
	assert_true(_classifier.is_core_tool("get_project_context"), "get_project_context should be available in the default loop")
	assert_eq(_classifier.get_tool_group("get_project_context"), "Project", "get_project_context should be in the Project group")

func test_apply_project_change_set_is_core_project_tool():
	assert_true(_classifier.is_core_tool("apply_project_change_set"))
	assert_eq(_classifier.get_tool_group("apply_project_change_set"), "Project")

func test_runtime_ui_semantics_is_debug_advanced():
	assert_true(_classifier.is_supplementary_tool("get_runtime_ui_semantics"))
	assert_eq(_classifier.get_tool_group("get_runtime_ui_semantics"), "Debug-Advanced")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 191, "Should have 191 supplementary tools")
...
## manifest 计数基线：225 总 / 30 core / 191 supplementary / 4 meta。
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 225, "manifest 应包含 225 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 30, "manifest 应有 30 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 191, "manifest 应有 191 个 supplementary 工具")

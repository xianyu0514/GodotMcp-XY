...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（242/28/208/6）
...
func test_all_231_tools_registered():
	...
	assert_eq(all_tools.size(), 242, "Should have exactly 242 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 208, "Should have 208 supplementary tools")
...
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 242, "manifest 应包含 242 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 28, "manifest 应有 28 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 208, "manifest 应有 208 个 supplementary 工具")

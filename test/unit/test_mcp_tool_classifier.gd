...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（245/28/211/6）
...
func test_all_231_tools_registered():
	...
	assert_eq(all_tools.size(), 245, "Should have exactly 245 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 211, "Should have 211 supplementary tools")
...
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 245, "manifest 应包含 245 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 28, "manifest 应有 28 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 211, "manifest 应有 211 个 supplementary 工具")

...
func test_all_196_tools_registered():
	var all_tools: Array = _classifier.get_all_tools()
	assert_eq(all_tools.size(), 196, "Should have exactly 196 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 166, "Should have 166 supplementary tools")

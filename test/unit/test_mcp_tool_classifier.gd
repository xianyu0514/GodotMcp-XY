...
#     manifest 不一致”漂移）
#   - test_manifest_counts：manifest 计数（232/28/198/6）
...
func test_all_231_tools_registered():
	...
	assert_eq(all_tools.size(), 232, "Should have exactly 232 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 198, "Should have 198 supplementary tools")
...
## manifest 计数基线：232 总 / 28 core / 198 supplementary / 6 meta。
func test_manifest_counts():
	assert_eq(ManifestScript.TOOLS.size(), 232, "manifest 应包含 232 个工具")
	assert_eq(ManifestScript.count_by_category("core"), 28, "manifest 应有 28 个 core 工具")
	assert_eq(ManifestScript.count_by_category("supplementary"), 198, "manifest 应有 198 个 supplementary 工具")

...
func _indexed_entry(sequence: int, message: String, payload: Variant) -> Dictionary:

	return {"sequence": sequence, "session_id": 0, "message": message,

		"data": [payload], "timestamp": 0.0}



func test_indexed_finder_returns_globally_earliest_match() -> void:

	var index: Dictionary = {

		"mcp:a": [_indexed_entry(3, "mcp:a", {"x": 3})],

		"mcp:b": [_indexed_entry(2, "mcp:b", {"x": 2})],

		"mcp:other": [_indexed_entry(1, "mcp:other", {"x": 1})],

	}

	var entry: Dictionary = MCPDebuggerBridge._earliest_captured_after(

		index, 0, ["mcp:a", "mcp:b"], [], {})

	assert_eq(int(entry.get("sequence", -1)), 2,

		"earliest matching entry wins regardless of bucket order")

	var none: Dictionary = MCPDebuggerBridge._earliest_captured_after(

		index, 2, ["mcp:other"], [], {})

	assert_true(none.is_empty(), "baseline filters older entries and unmatched names")



func test_indexed_finder_honors_match_fields_on_response_names() -> void:

	var index: Dictionary = {"mcp:runtime_info": [

		_indexed_entry(1, "mcp:runtime_info", {"fps": 60}),

		_indexed_entry(2, "mcp:runtime_info", {"fps": 120})]}

	var matched: Dictionary = MCPDebuggerBridge._earliest_captured_after(

		index, 0, ["mcp:runtime_info"], [], {"fps": 120})

	assert_eq(int(matched.get("sequence", -1)), 2,

		"field filter skips non-matching payloads")



func test_indexed_finder_does_not_field_filter_error_names() -> void:

	var index: Dictionary = {"mcp:error": [

		_indexed_entry(1, "mcp:error", {"code": "x"}),

		_indexed_entry(2, "mcp:error", {"code": "y"})]}

	var matched: Dictionary = MCPDebuggerBridge._earliest_captured_after(

		index, 0, [], ["mcp:error"], {"code": "y"})

	assert_eq(int(matched.get("sequence", -1)), 1,

		"error names keep the original linear semantics: no field filtering")

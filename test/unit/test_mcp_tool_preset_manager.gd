...
func test_minimal_core_enables_core_plus_meta_tools():
	...
	assert_eq(_count_enabled(states), 34, "minimal_core should enable 28 core + 6 always-on meta tools")
...
func test_meta_tools_survive_every_preset():
	for preset_id in _manager.get_preset_ids():
		...
		assert_true(states["search_tools"], "search_tools should stay enabled under " + preset_id)
		assert_true(states["get_tool_details"], "get_tool_details should stay enabled under " + preset_id)
...
func test_all_enables_everything():
	...
	assert_eq(_count_enabled(states), 251, "all preset should enable 251 tools")
...
func test_debugging_includes_core_plus_debug_advanced():
	...
	assert_eq(_count_enabled(states), 110, "debugging = 28 core + 6 meta + 76 Debug-Advanced (unchanged: new tools are Scene/Node groups)")
...
func test_level_design_enables_authoring_groups():
	...
	assert_eq(_count_enabled(states), 90, "level_design = 28 core + 6 meta + 8 + 9 + 14 + 23 advanced authoring tools (gridmap+csg)")

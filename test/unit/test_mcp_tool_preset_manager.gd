...
func test_all_enables_everything():
	...
	assert_eq(_count_enabled(states), 249, "all preset should enable 249 tools")
...
func test_level_design_enables_authoring_groups():
	...
	assert_eq(_count_enabled(states), 88, "level_design = 28 core + 6 meta + 8 + 9 + 12 + 23 advanced authoring tools")

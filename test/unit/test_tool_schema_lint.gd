...
## 工具数下限（历史基线 218；当前为 233 = 28 核心 + 199 补充 + 6 元）。
...
##   - 全量 233 工具 ≤ FULL_SET_TOKEN_BUDGET（60k token）。
...
const KNOWN_OVER_BUDGET_TOOLS: Dictionary = {
	...
	"apply_animation_preset": "动画预设四件套（2D 完胜）：fade/slide/shake/pulse 的参数语义与轨道路径推导必须内联（442 token）",
...
func test_cost_aware_workflow_avoids_most_full_load_schema_tokens() -> void:
	...
	assert_gte(float(route.get("estimated_token_savings_ratio", 0.0)), 0.90,
		"真实 233 工具目录的典型路线应避免至少 90% 的补充 schema token")

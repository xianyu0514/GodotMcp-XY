...
func test_claim_checkpoint_fail_and_resume_preserve_execution_state():
	var store := TaskPlanStore.new(TaskPlanStore.new_plan("Recoverable loop"))
	store.add_task({"id": "work", "title": "Build gameplay"})
	var claimed: Dictionary = store.claim_task("", "codex", 300)
	assert_eq(claimed.get("status"), "ok")
	assert_eq(claimed.get("task", {}).get("status"), "in_progress")
	assert_eq(claimed.get("task", {}).get("execution", {}).get("attempt"), 1)
	var checkpointed: Dictionary = store.checkpoint_task("work", "scene created", {"path": "res://main.tscn"}, 600)
	assert_eq(checkpointed.get("checkpoint", {}).get("data", {}).get("path"), "res://main.tscn")
	var resume: Dictionary = store.resumable("codex")
	assert_eq(resume.get("active", []).size(), 1)
	assert_eq(resume.get("active", [])[0].get("checkpoint", {}).get("note"), "scene created")
	var failed: Dictionary = store.fail_task("work", "compile failed", true, {"line": 8})
	assert_eq(failed.get("task", {}).get("status"), "pending")
	assert_eq(failed.get("failure", {}).get("data", {}).get("line"), 8)
	var reclaimed: Dictionary = store.claim_task("work", "codex", 300)
	assert_eq(reclaimed.get("task", {}).get("execution", {}).get("attempt"), 2)

func test_claim_rejects_active_lease_from_another_worker():
	var store := TaskPlanStore.new(TaskPlanStore.new_plan("Lease"))
	store.add_task({"id": "work", "title": "Build"})
	store.claim_task("work", "worker-a", 300)
	var conflict: Dictionary = store.claim_task("work", "worker-b", 300)
	assert_has(conflict, "error")
	assert_true(str(conflict.get("error", "")).contains("worker-a"))

func test_explicit_claim_respects_dependencies():
	var store := TaskPlanStore.new(TaskPlanStore.new_plan("Dependencies"))
	store.add_task({"id": "first", "title": "First"})
	store.add_task({"id": "second", "title": "Second", "depends_on": ["first"]})
	var blocked: Dictionary = store.claim_task("second", "codex", 300)
	assert_has(blocked, "error")
	assert_eq(blocked.get("blocked_by"), ["first"])

...
func test_initial_spawn_is_level_aware() -> void:
	var source: String = BlueprintsScript.controller_script(FULL_GAME)
	# 走廊几何：恒定基址 110 + 自适应间距（任何币数簇尾 <=188px，
	# 敌带危险缘前留余量；出生点磁吸区外）。
	assert_true(source.contains("var _coin_spacing: float = minf(40.0, 78.0 / float(maxi(COINS_TO_WIN - 1, 1)))"),
		"coin spacing adapts to the cluster size")
	assert_true(source.contains("var base_x: float = 110.0"),
		"the cluster base is corridor-fixed (spawn-safe, band-safe)")

...
func test_recoverable_execution_actions_round_trip():
	_call({"action": "init", "goal": "G"})
	_call({"action": "add_task", "title": "A", "id": "a"})
	var claimed: Dictionary = _call({"action": "claim", "worker": "codex", "lease_seconds": 300})
	assert_eq(claimed.get("task", {}).get("id"), "a")
	var checkpointed: Dictionary = _call({"action": "checkpoint", "id": "a", "note": "implemented", "data": {"commit": "abc"}})
	assert_eq(checkpointed.get("checkpoint", {}).get("data", {}).get("commit"), "abc")
	var resumed: Dictionary = _call({"action": "resume", "worker": "codex"})
	assert_eq(resumed.get("active", []).size(), 1)
	var failed: Dictionary = _call({"action": "fail", "id": "a", "error_message": "retry", "retryable": true})

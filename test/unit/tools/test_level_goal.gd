...
func test_level_legs_walk_three_levels() -> void:
	...
	assert_has(expressions, "str(current_level) + \"|\" + str(coins_collected == COINS_TO_WIN) + \"|\" + game_state + \"|\" + str(_last_restored.get(\"level\", 1)) + \"|\" + str(_last_restored.get(\"coins\", 0))=>3|true|win|1|0",
...
func test_level_legs_walk_both_levels() -> void:
	...
	assert_has(expressions, "str(current_level) + \"|\" + str(coins_collected == COINS_TO_WIN) + \"|\" + game_state + \"|\" + str(_last_restored.get(\"level\", 1)) + \"|\" + str(_last_restored.get(\"coins\", 0))",

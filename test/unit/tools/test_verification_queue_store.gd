...
func _pass_all(_item: Dictionary) -> Dictionary:
	return {"passed": true, "evidence": {"runner": "fake"}}

func _fail_even(item: Dictionary) -> Dictionary:
	# label 为奇数序号的项失败（detail.goal 的尾号奇偶决定）。
	var detail: Dictionary = item.get("detail", {})
	var goal: String = String(detail.get("goal", ""))
...
func test_advance_slices_without_losing_items() -> void:
	...
	var first: Dictionary = await StoreScript.advance(queue, 5, _pass_all)
	...
	var second: Dictionary = await StoreScript.advance(queue, 5, _pass_all)
	assert_eq(int(second["processed"]), 5)
	var third: Dictionary = await StoreScript.advance(queue, 5, _pass_all)
...
func test_zero_budget_advances_nothing() -> void:
	...
	var result: Dictionary = await StoreScript.advance(queue, 0, _pass_all)
...
func test_failure_blocks_completion_but_keeps_collecting() -> void:
	...
	var result: Dictionary = await StoreScript.advance(queue, 4, _fail_even)
...
func test_restart_resumes_from_persisted_state() -> void:
	...
	await StoreScript.advance(queue, 2, _pass_all)
	...
	var resumed: Dictionary = await StoreScript.advance(resumed_queue, 10, _pass_all)
...
func test_watch_drift_invalidates_collected_evidence() -> void:
	...
	await StoreScript.advance(queue, 3, _pass_all)
...
func test_terminal_queue_refuses_advance() -> void:
	...
	await StoreScript.advance(queue, 1, _pass_all)
	var again: Dictionary = await StoreScript.advance(queue, 1, _pass_all)

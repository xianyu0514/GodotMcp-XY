...
class FakeTunnelManager extends MCPTunnelManager:
	...
	func _read_process_command_line(process_id: int) -> String:
		...
		return '"%s" tunnel --logfile "%s"' % [spawned_binary, get_log_path()]
...
func test_launch_args_write_logs_without_parent_owned_pipes() -> void:
	var log_path: String = _tmp_root.path_join("cloudflared.log")
	var args: PackedStringArray = MCPTunnelManagerScript.build_launch_args(9080, log_path)
	assert_eq(args[0], "tunnel")
	assert_true(args.has("--no-autoupdate"), "Pinned connector must not replace itself")
	assert_true(args.has("--logfile"), "Detached tunnel output must go to a durable file")
	assert_eq(args[args.find("--logfile") + 1], log_path)
...
func test_start_persists_url_and_detach_keeps_process_alive() -> void:
	...
	assert_eq(mgr.spawned_binary, "/opt/cloudflared")
...
func test_restore_adopts_same_process_and_explicit_stop_terminates_it() -> void:
	...
	restored.spawned_binary = "/opt/cloudflared"
	restored.fake_command_line = '"/opt/cloudflared" tunnel --logfile "%s"' % restored.get_log_path()
	...
	restored.stop()
	assert_eq(restored.killed_pid, restored.fake_pid, "Only the explicit Stop action should kill the tunnel")

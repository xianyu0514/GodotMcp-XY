...
- 30 个核心工具默认启用。
- 190 个高级工具默认注册但不启用，可在面板或通过 `enable_tools` 开启。
...
`get_project_context` 与可订阅的 `godot://project/context` 资源提供带版本的紧凑定向快照；成功的写工具会通知已订阅资源刷新。
`visual_playtest` 将启动、运行时断言、截图、基线比较和清理压缩为一个结论。MCP 补全及脚本/场景资源模板可减少发现往返。

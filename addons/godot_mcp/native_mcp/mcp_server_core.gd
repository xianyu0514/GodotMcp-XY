...
const SERVER_INSTRUCTIONS: String = "Godot MCP starts with ~30 core tools plus four always-on meta tools so tools/list stays small. Most editing tasks work with the default set. For one missing capability call search_tools with mode='tools'. For a multi-step goal call search_tools with mode='workflow': its local deterministic router covers the intent across inspect/execute/verify stages with at most 8 tool names and no duplicated schemas. Flatten those stage names into one enable_tools request, then use the refreshed tools/list schemas. Every non-meta atomic tool is routable, so do not give up and do not load the full 221-tool catalog. Call get_tool_details only when comparing candidates or when a client cannot refresh. Use list_tool_catalog with summary_only=true only to browse group counts and workflow coverage. Prefer focused presets (game_2d, game_3d, ui_localization, gameplay_scripting, animation_audio, release_export, level_design, debugging, automation_qa, art_resources, minimal_core); 'all' has the highest context cost. Reuse catalog_revision with known_revision to avoid downloading an unchanged catalog."

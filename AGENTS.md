...
## 项目简介
一个 **Godot 4.7 EditorPlugin**（位于 `addons/godot_mcp/`），在 Godot 内部原生实现了 MCP（Model Context Protocol）服务器，无需 Node.js 依赖。提供 **243 个工具**（28 核心 + 209 补充 + 6 元工具），分为 6 大类（外加始终在线的 Meta 元工具组），供 AI 助手读取和修改项目。运行期地图探针（mcp_runtime_probe.gd）自 M5 起双兼容 TileMap 与 TileMapLayer（区域读取/批量写入/立即内部更新）。
...
│   ├── tools_manifest.gd       # 单一数据表（唯一真相）：243 个工具 name → {category, group}
...
> 工具总数以 `tools_manifest.gd` 为唯一真相（当前 243 = 28 core + 209 supplementary + 6 meta）；上表每文件计数为该文件注册的工具处理器数量，横跨文件的分组计数（README 表格）以 manifest 为准。

...
- **204 个工具，分级合理** —— 30 个高价值「核心」工具默认开启；另有 172 个「高级」工具按需一键启用，外加 2 个常驻「元」工具用于工具发现。
...
插件提供 **204 个工具**，分为六大类（外加 2 个用于工具发现的常驻「元」工具），每一类都有完整的逐工具参考。
...
默认仅启用 30 个**核心**工具（外加 2 个常驻**元**工具）；其余 172 个**高级**工具可在 MCP 面板中按需启用。详见

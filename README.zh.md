...
- **206 个工具，分级合理** —— 30 个高价值「核心」工具默认开启；另有 174 个「高级」工具按需一键启用，外加 2 个常驻「元」工具用于工具发现。
...
插件提供 **206 个工具**，分为六大类（外加 2 个用于工具发现的常驻「元」工具），每一类都有完整的逐工具参考。
...
| [调试与运行时 Debug](docs/tools/debug-tools.md) | 71 | 日志、调试器、断点、性能分析、实时运行时控制 |
| [项目 Project](docs/tools/project-tools.md) | 55 | 设置、资源、输入映射、审计、4.7 迁移、资产生成、任务计划 |
| [元 Meta](docs/tools/README.md#meta-tools-tool-discovery) | 2 | 常驻的工具发现与按需启用（`list_tool_catalog`、`enable_tools`） |

默认仅启用 30 个**核心**工具（外加 2 个常驻**元**工具）；其余 174 个**高级**工具可在 MCP 面板中按需启用。详见

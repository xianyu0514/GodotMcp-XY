...
- **221 个工具且默认面精简：** 28 个核心工具默认启用，189 个高级工具按需启用，另有 4 个常驻元工具负责发现和启用工具。
...
启动时只有核心工具和元工具会出现在 `tools/list` 中。最快路径是一次 `enable_tools({"workflow_query": "..."})` 调用：纯本地不可变能力索引把中英文目标路由为“检查 → 执行 → 验证”，并在同一次原子切换中从全部 217 个非 Meta 原子工具里启用最多 8 个，不复制 Schema。每个原子工具都可通过精确名称、官方英文说明或中文说明直接命中。每个工具定义只在注册时估算一次 Schema token 成本；新增意图覆盖相同时，路由器优先选择“语义价值/增量 token”最高的候选，而精确原子请求始终拥有最高优先级。归一化任务目标复用最多 64 项的 LRU 路由缓存。工具显隐不会重建索引或清空路线，只有工具定义增删才失效，因此切换任务后仍保持高缓存命中。核心/元工具始终保留，默认替换上一任务的高级工具，使后续提示词更小且更利于缓存命中；仍支持增量装载、手工分组和 12 个预设。`search_tools` 保留用于预览路线和比较候选。目录 revision 保持发现响应精简；依赖标签缓存 revision 则保留无关的场景、脚本和资源读取，并按脚本/资源路径懒失效。详见 [Tools Reference](docs/tools/README.md)。
